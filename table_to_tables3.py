...
data['deleted'] = 0  # Beispielwert, anpassen nach Bedarf
...
# Fehlende Spalten für Tabelle 2 hinzufügen, falls nicht vorhanden
...
for col in columns_needed_tabelle2:
    if col not in data.columns:
        data[col] = None  # oder setzen Sie einen Standardwert, z.B. 0 oder ''

# Tabelle 2 erstellen
data['mess_id_z'] = mess_id_z
data['messgeraet_id_z'] = 1
tabelle2_columns = ['timestamp', 'mess_id_z', 'stab_id', 'messgeraet_id_z', 'messreihe_id'] + columns_needed_tabelle2 + ['deleted']
...
tabelle3_columns = ['timestamp', 'mess_id_i_u', 'stab_id', 'messgeraet_id_i_u', 'messreihe_id'] + columns_needed_tabelle3 + ['deleted']
